...
### main function 
runAnalysis_R <- function(work_path, input_file, output_file, log_file, cor_file) 
{
  ...
  ## use file.create to prevent override the pre-existing file
  file.create(file=output_file )
  ...
  while(length(aLine <- readLines(fileconn,n=1)) > 0){
    ...
    if (all(nonNA)) {
      ...
      if (isdiag){
        ...
        Q = round(Q,2)
        write(c(rsid, nstudy, LSs, LSse, LSp, Isq, Q, Qp, stat1, stat2, RE2Cp_cond, RE2Cp),file=output_file,ncolumns = 12,append=T)
      } else {
        Q <- NA;  Qp <- NA;  Isq <- NA
        write(c(rsid, nstudy, LSs, LSse, LSp, stat1, stat2, RE2Cp_cond, RE2Cp),file=output_file,ncolumns = 9,append=T)
      }
      ...
      if (isdiag){
        ...
        Q = round(Q,2)
        write(c(rsid, temp_ns, LSs, LSse, LSp, Isq, Q, Qp, stat1, stat2, RE2Cp_cond, RE2Cp),file=output_file,ncolumns = 12,append=T)
      } else {
        Q <- NA;  Qp <- NA;  Isq <- NA
        write(c(rsid, temp_ns, LSs, LSse, LSp, stat1, stat2, RE2Cp_cond, RE2Cp),file=output_file,ncolumns = 9,append=T)
      }
...
runAnalysis <- cmpfun(runAnalysis_R ,options=list(optimize=2,suppressAll=T,suppressUndefined=T))
runAnalysis(work_path,input_file, output_file,log_file,cor_file)
